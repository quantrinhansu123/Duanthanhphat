...
/**
 * Kế hoạch tự chia đều đã lưu có thể còn giữ "ngày nghỉ cũ" = 0 mối hàn sau khi
 * danh sách ngày nghỉ bị thất lạc (lỗi cũ: API metadata bỏ mất offDays). Nếu kế
 * hoạch đã lưu ĐÚNG bằng công thức chia đều theo chính tập ngày 0 của nó, coi là
 * kế hoạch tự sinh và dựng lại theo offDays hiện tại. Kế hoạch nhập từ Excel
 * (phân bố bất kỳ) sẽ không khớp công thức nên được giữ nguyên.
 */
function reconcileAutoPlan(
  storedPlan: TheoreticalProgressRow[],
  total: number,
  startDate: string,
  endDate: string,
  offDays: string[],
): TheoreticalProgressRow[] {
  if (storedPlan.length === 0) return storedPlan;

  const currentOff = clampOffDaysToRange(offDays, startDate, endDate);
  const zeroDays = storedPlan.filter((row) => row.so_moi_han === 0).map((row) => row.ngay);
  const zeroSet = new Set(zeroDays);
  const sameAsOffDays =
    zeroSet.size === currentOff.length && currentOff.every((day) => zeroSet.has(day));
  if (sameAsOffDays) return storedPlan;

  const rebuiltFromZeros = buildDailyWeldPlan(total, startDate, endDate, zeroDays);
  const looksAutoGenerated =
    rebuiltFromZeros.length === storedPlan.length &&
    rebuiltFromZeros.every(
      (row, index) =>
        row.ngay === storedPlan[index].ngay && row.so_moi_han === storedPlan[index].so_moi_han,
    );
  if (!looksAutoGenerated) return storedPlan;

  return buildDailyWeldPlan(total, startDate, endDate, currentOff);
}
