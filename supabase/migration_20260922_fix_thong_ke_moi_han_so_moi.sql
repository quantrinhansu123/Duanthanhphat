-- ============================================================
-- Fix: column "so_moi" of relation "thong_ke_moi_han_theo_ngay"
--      does not exist
--
-- Nguyên nhân (Vercel / Supabase production):
--   - Bảng còn schema cũ (so_luong_thuc_hien, so_luong_loi)
--   - Hàm trigger sync đã dùng cột mới (so_moi, so_dat, ...)
-- → INSERT/UPDATE nhật ký hàn fail.
--
-- Cách xử lý: nâng bảng lên schema đếm dòng (so_moi...),
-- tạo lại trigger SECURITY DEFINER + RLS anon, backfill.
-- Có thể chạy lại an toàn.
-- ============================================================

create extension if not exists "pgcrypto";

-- 1) Drop trigger trước khi đụng bảng / hàm
drop trigger if exists trg_sync_thong_ke_moi_han_theo_ngay on public.lich_su_moi_han;

-- 2) Nếu thiếu cột so_moi → rebuild bảng (dữ liệu tổng hợp sẽ backfill lại)
do $$
begin
  if to_regclass('public.thong_ke_moi_han_theo_ngay') is null
     or not exists (
       select 1
       from information_schema.columns
       where table_schema = 'public'
         and table_name = 'thong_ke_moi_han_theo_ngay'
         and column_name = 'so_moi'
     )
  then
    drop view if exists public.thong_ke_moi_han_theo_ngay_du_an;
    drop table if exists public.thong_ke_moi_han_theo_ngay cascade;
    ...
    comment on table public.thong_ke_moi_han_theo_ngay is
      'Tổng hợp số mối hàn theo ngày/dự án/công nghệ/loại (đếm dòng nhật ký), nuôi bằng trigger trên lich_su_moi_han.';
    ...
    -- Bổ sung cột còn thiếu (nếu schema nửa vời)
    alter table public.thong_ke_moi_han_theo_ngay
      add column if not exists so_dat bigint not null default 0,
      add column if not exists so_khong_dat bigint not null default 0,
      add column if not exists so_cho_tn bigint not null default 0,
      add column if not exists so_khong_tn bigint not null default 0,
      add column if not exists so_han_lai bigint not null default 0,
      add column if not exists updated_at timestamptz not null default now();
  end if;
end $$;

-- 3) Hàm sync (SECURITY DEFINER — anon có thể ghi nhật ký)
...
grant select, insert, update, delete on public.thong_ke_moi_han_theo_ngay to anon, authenticated;

-- 5) Backfill từ lich_su_moi_han
