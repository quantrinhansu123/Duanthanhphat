...
create table if not exists public.thiet_bi (
  ...
  cong_nghe_han text,
  loai_ray_ho_tro text,
  nang_suat_han text,
  gio_hoat_dong numeric(10,2) default 0,
  tong_moi_han integer default 0,
  vi_tri_hien_tai text not null default 'Chưa cập nhật',
  du_an_hien_tai text,
  nguoi_phu_trach text,
  to_van_hanh text,
  ngay_bao_tri_gan_nhat text,
  ngay_bao_tri_tiep_theo text,
  ghi_chu     text,
  thong_so    jsonb default '{}'::jsonb,
  hinh_anh    text,                        -- URL Storage
  hinh_anh_chi_tiet text[] default '{}',
