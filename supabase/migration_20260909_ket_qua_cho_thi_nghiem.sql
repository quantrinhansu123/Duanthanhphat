-- ============================================================
-- Sửa logic "Kết quả" của lịch sử mối hàn: mối đang "Chờ thí nghiệm"
-- KHÔNG được coi là "Đạt". Trước đây view chỉ xét ghi_chu + so_luong_loi
-- nên mọi mối chưa có kết quả (so_luong_loi = 0) đều bị tính là Đạt / 0 lỗi.
--
-- LƯU Ý: giữ nguyên THỨ TỰ 23 cột hiện có của view (create or replace view
-- không cho phép đổi vị trí cột), chỉ THÊM 2 cột mới ở cuối:
--   - tinh_trang_thi_nghiem
--   - created_at   (dùng để sắp xếp danh sách theo thời điểm tạo)
...
select
  ...
  case
    ...
    when ls.ghi_chu ilike '%Kết quả: Không đạt%' or coalesce(ls.so_luong_loi, 0) > 0
      then 'Không đạt'
    -- Mối chưa có kết quả thí nghiệm → giữ nguyên trạng thái chờ, không tính là Đạt.
  ...
  -- ↓↓↓ 2 cột mới, thêm ở CUỐI để không đổi vị trí cột cũ ↓↓↓
...
-- ------------------------------------------------------------
-- RPC thống kê: thêm cột cho_thi_nghiem để dashboard hiển thị đúng.
-- ------------------------------------------------------------
drop function if exists public.thong_ke_lich_su_moi_han(
  date, date, text, text, text[], text[], text[], text[], text[], text
);

create function public.thong_ke_lich_su_moi_han(
...
as $$
  ...
  totals as (
    select
      ...
      count(*) filter (where ket_qua = 'Chờ thí nghiệm') as cho_thi_nghiem
    from filtered
  ),
  accounting as (
    select coalesce(
  ...
  select totals.tong, totals.dat, totals.khong_dat, totals.sua_chua, totals.cho_thi_nghiem, accounting.payload
